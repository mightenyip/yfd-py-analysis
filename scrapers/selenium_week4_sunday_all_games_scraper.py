#!/usr/bin/env python3
"""
Advanced Selenium scraper for Yahoo Daily Fantasy Week 4 Sunday data from ALL game times.
...
def scrape_all_week4_sunday_games():
    """
    Main function to scrape Week 4 Sunday data from ALL game times.
    """
...
def save_all_week4_sunday_data(player_data, filename="week4_Sunday_all_games.csv"):
    """Save the Week 4 Sunday data from all games with correct Points column."""
    if not player_data:
    ...
    print(f"✅ Week 4 Sunday data from all games saved to {filepath}")
    print(f"📊 Found {len(player_data)} valid players from all games")
    
    # Display summary
    if len(player_data) > 0:
        print(f"\n📋 Sample of Week 4 Sunday data from all games:")
...
def main():
    print("Yahoo Daily Fantasy Week 4 Sunday Data Scraper - ALL GAMES")
    print("=" * 70)
    print("This will capture data from ALL available game times via dropdown.")
    print("Data will be saved as 'week4_Sunday_all_games.csv' in the data_csv directory.")
    print("=" * 70)
    
    # Scrape Week 4 Sunday data from all game times
    all_player_data = scrape_all_week4_sunday_games()
    
    if all_player_data:
        # Save Week 4 Sunday data from all games
        df = save_all_week4_sunday_data(all_player_data)
        
        if df is not None:
            print(f"\n🎉 SUCCESS! Scraped {len(df)} players from all Week 4 Sunday games")
            print("📁 Data saved to: data_csv/week4_Sunday_all_games.csv")
        else:
            print("\n❌ Failed to save data")
    else:
        print("\n❌ No data found")
