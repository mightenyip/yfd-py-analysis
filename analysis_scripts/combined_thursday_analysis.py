#!/usr/bin/env python3
"""
Combined Analysis of All Thursday Night Football Data (Week 2-4)
Analyzing salary vs points correlation across all Thursday games
"""
...
def calculate_overall_correlation(df_active):
    ...
    print(f"\n📈 OVERALL THURSDAY CORRELATION ANALYSIS:")
...
def find_top_performers(df_active):
    """Find top performers and value plays across all Thursdays."""
    # Calculate points per dollar
    df_active['points_per_dollar'] = df_active['points_numeric'] / df_active['salary_numeric']
    
    print(f"\n💎 TOP 10 VALUE PLAYS (All Thursdays - Highest Points per Dollar):")
    ...
    print(f"\n🏆 TOP 10 HIGHEST SCORING GAMES (All Thursdays):")
    ...
    print(f"\n💸 TOP 10 BUSTS (Min $20 salary - All Thursdays):")
    min_salary = 20
...
def create_comprehensive_visualizations(df_active, slope, intercept):
    """Create comprehensive visualizations for all Thursday data."""
    ...
    fig.suptitle('Thursday Night Football: Salary vs Points Analysis (Week 2-4)', fontsize=16, fontweight='bold')
    ...
    plt.savefig(f"{plots_dir}/combined_thursday_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"📊 Comprehensive plot saved to: {plots_dir}/combined_thursday_analysis.png")

def analyze_thursday_patterns(df_active):
    """Analyze specific Thursday Night Football patterns."""
    print(f"\n🔍 THURSDAY NIGHT FOOTBALL PATTERNS:")
...
def compare_with_monday_data():
    """Compare Thursday correlation with Monday correlation from previous analysis."""
    print(f"\n🔄 COMPARISON WITH MONDAY NIGHT FOOTBALL:")
    print(f"   Monday Night Football (Week 1-3): r = 0.569, R² = 0.324")
    print(f"   Thursday Night Football (Week 2-4): Analysis in progress...")
    print(f"   This will show which game type has stronger salary correlation")

def main():
    print("Thursday Night Football Combined Analysis (Week 2-4)")
    print("=" * 70)
    print("Analyzing salary vs points correlation across all Thursday games")
    print("=" * 70)
    ...
    # Analyze Thursday patterns
    analyze_thursday_patterns(df_active)
    
    # Compare with Monday data
    compare_with_monday_data()
    
    # Final conclusions
    print(f"\n🎯 THURSDAY NIGHT FOOTBALL CONCLUSIONS:")
    print(f"   • Total games analyzed: 3 Thursday Night Football games")
    print(f"   • Total active players: {len(df_active)}")
    print(f"   • Overall correlation: {correlation[0]:.3f} (salary explains {r_squared:.1%} of variance)")
    
    if correlation[0] > 0.5:
        print(f"   • STRONG correlation - Thursday Night Football shows clear salary/performance relationship")
    elif correlation[0] > 0.3:
        print(f"   • MODERATE correlation - Salary has some predictive value on Thursday nights")
    else:
        print(f"   • WEAK correlation - Thursday Night Football is more unpredictable")
