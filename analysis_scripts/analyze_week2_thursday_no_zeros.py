#!/usr/bin/env python3
"""
Analysis of Week 2 Thursday salary vs points relationship - EXCLUDING ZERO POINT PLAYERS.
...
def load_and_clean_data():
    """Load and clean the Week 2 Thursday data, excluding zero-point players."""
    print("📊 Loading Week 2 Thursday data (excluding zero-point players)...")
    ...
    # EXCLUDE ZERO POINT PLAYERS
    df_active = df_clean[df_clean['points_clean'] > 0].copy()
    
    print(f"✅ Original dataset: {len(df_clean)} players")
    print(f"✅ After removing zero-point players: {len(df_active)} players")
    print(f"📈 Salary range: ${df_active['salary_clean'].min():.0f} - ${df_active['salary_clean'].max():.0f}")
    print(f"📈 Points range: {df_active['points_clean'].min():.1f} - {df_active['points_clean'].max():.1f}")
    
    return df_active

def explore_relationship(df):
    """Explore the salary vs points relationship."""
    print("\n🔍 Exploring Salary vs Points Relationship (Active Players Only)...")
...
def create_visualizations(df, models):
    ...
    fig.suptitle('Week 2 Thursday: Salary vs Points Analysis (Active Players Only)', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('Salary vs Points with Model Fits (Active Players)')
    ax1.legend()
    ...
    ax2.set_title('Value Efficiency by Salary (Active Players)')
    ax2.legend()
    ...
    ax3.set_title('Model Residuals (Active Players)')
    ...
    ax4.set_title('Value Efficiency by Position (Active Players)')
    ...
    # Save the plot
    output_path = '/Users/mightenyip/Documents/GitHub/yfd-py-test/plots_images/week2_thursday_no_zeros_analysis.png'
...
def draw_conclusions(df, models, high_salary_analysis, position_stats):
    ...
    print("\n🎯 FINAL CONCLUSIONS (ACTIVE PLAYERS ONLY)")
    print("=" * 60)
...
def main():
    ...
    print("Week 2 Thursday Salary vs Points Analysis - ACTIVE PLAYERS ONLY")
    print("Testing Hypothesis: Parabolic function with >$20 players as sunk costs")
    print("=" * 70)
    
    # Load and clean data (excluding zero-point players)
